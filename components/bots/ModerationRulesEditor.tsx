    ...
    <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      <div className="flex items-start justify-between mb-4">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-lg transition-colors ${showTester ? `${bgColor} border` : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
        >
          ...
          {testText && (
            <p className="mt-2 text-xs">
              {matchedPatterns.length > 0 ? (
                ...
                <span className="text-green-700 font-medium">No matches — comment would pass through.</span>
              )}
            </p>
          )}
        </div>
      ...
      <div className="flex gap-2 mb-4">
        <Input
          placeholder="e.g. muy caro or regex like put[ao]"
          value={input}
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 bg-slate-800 hover:bg-slate-900 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50 flex-shrink-0"
        >
      ...
      {patterns.length === 0 ? (
        ...
        <div className="flex flex-wrap gap-2">
          {patterns.map(p => (
            <span
              key={p}
              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-mono font-medium ${badgeBg}`}
            >
              {p}
              <button
  ...
  return (
    <div className="space-y-4">
      <PatternList
        ...
        description="Comments matching these patterns will be permanently deleted."
      ...
      <PatternList
        title="Spam Keywords"
        description="Comments matching these patterns will be hidden (not deleted)."
