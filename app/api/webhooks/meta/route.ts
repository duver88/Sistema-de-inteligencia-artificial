...
async function processWebhookAsync(rawBody: Buffer): Promise<void> {
  try {
    ...
    // Compact diagnostic: shape of each change (why it's accepted/discarded).
    // Does NOT log the commenter's name/message — end-user PII must not sit in
    // plaintext server logs.
    ...
    for (const entry of ((body.entry ?? []) as unknown as RawEntry[])) {
      for (const ch of entry.changes ?? []) {
        ...
        console.log(
          ...
          `parent_id=${v.parent_id ?? '-'} · post_id=${v.post_id ?? '-'} · comment_id=${v.comment_id ?? '-'}`
        );
      }
    }
